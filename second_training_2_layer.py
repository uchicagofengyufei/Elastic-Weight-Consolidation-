...
def build_net(input_var,pass_weight):
    ...
    return l_out, l_hid_0.W,l_out.W
...
#X_train_1 = X_train
#X_val_1 = X_val
X_train_1 = preprocessing.digit_rotate(X_train,30)
X_val_1 = preprocessing.digit_rotate(X_val,30)

#ind = range(0, 784)
#numpy.random.shuffle(ind)
#X_train_1 = preprocessing.digit_permute(X_train,ind)
#_val_1 = preprocessing.digit_permute(X_val,ind)
#X_val_1 = preprocessing.permute_mnist(X_val)
print(X_val_1.dtype)
...
network, W_n0, W_n1 = build_net(input_var,pass_weight)
...
loss_remember = (I0*lasagne.objectives.squared_error(W0,W_n0)).sum()+ \
                (I1*lasagne.objectives.squared_error(W1,W_n1)).sum()

loss = loss_class + 0*loss_remember
...
train_fn = theano.function([input_var, target_var,W0,W1,I0,I1], [loss,loss_class, loss_remember], updates=updates)
...
for epoch in range(num_epochs):
    ...
    for batch in mini_batch.iterate_minibatches(X_train_1, y_train, 500, shuffle=True):
        ...
        err, cls,rem = train_fn(inputs, targets,p_param[0],p_param[1],
